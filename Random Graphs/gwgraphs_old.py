...
def gw_barycenter_class_alg(distance_matrices,
    ...
    for i, (train_index,
        ...
        gw, log_dists = ot.gromov.gromov_wasserstein(barycenter_matrix_class1,
                                                     ...
                                                     loss_fun='square_loss',
                                                     log=True)
        print("Distance between barycenters: ", np.sqrt(np.abs(log_dists['gw_dist'])),
              f" Fold {i}")
        correct_count = 0
        ...
        for idx, graph in enumerate(X_test):
            ...
            predicted_label = -1 if np.abs(log_class1['gw_dist']) < np.abs(log_class2[
                'gw_dist']) else 1
    ...
    for i, (train_index,
        ...
        for idx, graph in enumerate(X_test):
            ...
            print("GW distance between the two barycenters: ",
                  np.sqrt(np.abs(log_bary_gw['gw_dist'])))
        ...
        for test_graph, ground_truth_label in zip(X_test, y_test):
            ...
            for train_graph, train_label in zip(X_train, y_train):
                gw, log = ot.gromov.gromov_wasserstein(test_graph,
                                                       ...
                                                       log=True,
                                                       random_seed=seed)
                distances.append((np.abs(log['gw_dist']), train_label))
