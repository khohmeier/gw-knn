...
def log_comprehensive_metrics(logger, y_true, y_pred, prefix=""):
    """Log comprehensive classification metrics including by-class accuracies and F1 scores
    ...
    logger.info(f'{prefix}Overall Accuracy: {accuracy:.4f}')
    logger.info(f'{prefix}F1 Score (Macro): {f1_macro:.4f}')
    logger.info(f'{prefix}F1 Score (Weighted): {f1_weighted:.4f}')
    logger.info(f'{prefix}By-class metrics:')
    
    for class_label, metrics in class_report.items():
        if isinstance(metrics, dict) and 'precision' in metrics:
            logger.info(f'{prefix}  Class {class_label}: Precision={metrics["precision"]:.4f}, '
