...
def do_linesearch(cost,
    ...
    if amijo:
        ...
        a = -2 * reg * np.sum(
            dot12 *
            deltaG)  #-2*alpha*<C1 dt C2,dt> si qqlun est pas bon c'est lui
        b = np.sum((M + reg * constC) * deltaG) - 2 * reg * (
            np.sum(dot12 * G) + np.sum(np.dot(C1, G).dot(C2) * deltaG))
        c = cost(G)  #f(xt)
...
def cg(a,
    ...
    while loop:
        ...
        alpha, fc, f_val = do_linesearch(cost=cost,
                                         G=G,
                                         deltaG=deltaG,
                                         Mi=Mi,
                                         f_val=f_val,
        ...
        if verbose:
            if it % 20 == 0:
                print(
                    ...
                    '\n' + '-' * 32)
            print('{:5d}|{:8e}|{:8e}|{:5e}'.format(it, f_val, delta_fval,
                                                   alpha))
