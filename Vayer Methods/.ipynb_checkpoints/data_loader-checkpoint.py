...
def build_MUTAG_dataset(path,one_hot=False):
    ...
    node_dic=node_labels_dic(path,'MUTAG_node_labels.txt') # ya aussi des nodes attributes
    data=[]
    for i in graphs:
        g=Graph()
        for node in data_dict[i[0]]:
...
def build_PTC_dataset(path,one_hot=False):
    ...
    node_dic=node_labels_dic(path,'PTC_MR_node_labels.txt') # ya aussi des nodes attributes
    data=[]
    for i in graphs:
        g=Graph()
        for node in data_dict[i[0]]:
