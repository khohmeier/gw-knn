...
"""
Gromov-Wasserstein barycenter classification algorithms
"""

def gw_barycenter_class_alg(distance_matrices,
                            labels,
                            num_folds,
                            num_nodes,
                            seed=42,
    ...
    for i, (train_index,
        ...
        if log is True:
            print("GW barycenter for Class 1 graphs:",
                  barycenter_matrix_class1, f"Fold {i}")
        ...
        if log is True:
            print("GW barycenter for Class 2 graphs:",
                  barycenter_matrix_class2, f"Fold {i}")
        ...
        # Calculate F1 score
        # Use macro here since we assume that the two classes have the same number of obs
        f1_macro = f1_score(
            true_labels, predicted_labels, average='macro') * 100

        print(f"F1 Macro: {f1_macro}%, Fold {i}")
        print(f"Classification Accuracy: {classification_accuracy}%",
              f"Fold {i}")
        print(f"Classification Accuracy for Class 1: {class1_accuracy}%",
              f"Fold {i}")
        print(f"Classification Accuracy for Class 2: {class2_accuracy}%",
              f"Fold {i}")
...
def gw_barycenter_class_alg_stratkfold(distance_matrices,
                                       labels,
                                       num_folds,
                                       num_nodes,
                                       seed=42,
    ...
    for i, (train_index,
        ...
        # Calculate F1 scores
        # Use weighted F1 since the classes have different numbers of obs
        f1_weighted = f1_score(
            true_labels, predicted_labels, average='weighted') * 100
...
def gw_barycentersubset_class_alg(distance_matrices,
    ...
    for i, (train_index,
        ...
        for idx, graph in enumerate(X_test):
            ...
            print("GW distance between test graph and Barycenter 1: ",
                  {log_class1['gw_dist']})
            print("GW distance between test graph and Barycenter 2: ",
                  {log_class2['gw_dist']})
            print("GW distance between the two barycenters: ",
                  log_bary_gw['gw_dist'])
        ...
        print(f"F1 Macro: {f1_weighted}%, Fold {i}")
...
def gw_barycenter_class_alg_var_nodes_stratkfold(distance_matrices,
                                                 labels,
                                                 num_folds,
                                                 num_nodes_class1,
                                                 num_nodes_class2,
...
def fgw_barycenter_class_alg_var_nodes_stratkfold(distance_matrices,
                                                  graphs_list,
                                                  labels,
                                                  num_folds,
                                                  num_nodes_class1,
    ...
    for i, (train_index,
        ...
        for graph_idx in class1_indices:
            graph = graphs_list[graph_idx]
            graph_features = np.array(
                [graph.nodes[n]['attributes'] for n in graph.nodes()])
            features_class1.append(graph_features)

        # Create uniform distributions for each graph
        ps_class1 = [
            np.ones(len(features)) / len(features)
            for features in features_class1
        ]
        lambdas_class1 = np.ones(len(features_class1)) / len(features_class1)

        barycenter_features_class1, barycenter_matrix_class1, log_class1 = ot.gromov.fgw_barycenters(
            N=num_nodes_class1,
            Ys=features_class1,
            Cs=X_train_class1,
            ps=ps_class1,
            lambdas=lambdas_class1,
            loss_fun='square_loss',
            random_state=seed,
            log=True)
        if log is True:
        ...
        # Get node attributes for class 2
        features_class2 = []
        class2_indices = [
            train_index[j] for j in range(len(X_train)) if y_train[j] == 1
        ]
        for graph_idx in class2_indices:
            graph = graphs_list[graph_idx]
            graph_features = np.array(
                [graph.nodes[n]['attributes'] for n in graph.nodes()])
            features_class2.append(graph_features)

        # Create uniform distributions for each graph
        ps_class2 = [
            np.ones(len(features)) / len(features)
            for features in features_class2
        ]
        lambdas_class2 = np.ones(len(features_class2)) / len(features_class2)

        barycenter_features_class2, barycenter_matrix_class2, log_class2 = ot.gromov.fgw_barycenters(
            N=num_nodes_class2,
            Ys=features_class2,
            Cs=X_train_class2,
            ps=ps_class2,
            lambdas=lambdas_class2,
            loss_fun='square_loss',
            random_state=seed,
            log=True)
        class2_barys.append(barycenter_matrix_class2)
        ...
        M = ot.dist(barycenter_features_class1, barycenter_features_class2)
        ...
        for idx, graph in enumerate(X_test):
            ...
            M1 = ot.dist(np.array(test_features), barycenter_features_class1)
            M2 = ot.dist(np.array(test_features), barycenter_features_class2)

            fgw_class1, log_class1 = ot.gromov.fused_gromov_wasserstein(
                M1,
                graph,
                barycenter_matrix_class1,
                loss_fun='square_loss',
                log=True)
            fgw_class2, log_class2 = ot.gromov.fused_gromov_wasserstein(
                M2,
                graph,
                barycenter_matrix_class2,
                loss_fun='square_loss',
            ...
            true_labels.append(ground_truth_label)
            # Check if classification is correct
            predicted_label = -1 if log_class1['fgw_dist'] < log_class2[
                'fgw_dist'] else 1
            predicted_labels.append(predicted_label)
            if ground_truth_label == -1:
                total_count_class1 += 1
...
def fgw_barycenter_class_alg_var_nodes_kfold(distance_matrices,
                                             graphs_list,
                                             labels,
                                             num_folds,
                                             num_nodes_class1,
    ...
    start = time.time()
    for i, (train_index,
            test_index) in enumerate(skf.split(distance_matrices)):
        print(f"Fold {i}:")
        if log is True:
            print(f"  Train: index={train_index}")
            print(f"  Test:  index={test_index}")
...
def gw_knn(distance_matrices,
    ...
    for i, (train_index,
        ...
        for k in k_list:
            ...
            print(
                f"k={k}, F1 Score ({average_type} avg): {f1:.2f}%, Fold {i}"
            )
    ...
    print(f"Overall Accuracy: {k_avg_accuracies[best_k]['overall']:.2f}%")
    print(f"F1 Score ({average_type} avg): {k_avg_f1_scores[best_k]:.2f}%")
    print(f"Class 1 Accuracy: {k_avg_accuracies[best_k]['class_1']:.2f}%")
    print(f"Class -1 Accuracy: {k_avg_accuracies[best_k]['class_-1']:.2f}%")
...
def fgw_knn(distance_matrices,graphs_list,labels,num_folds,k_list,method,seed=42,log_knn=False):
    class_acc_dict = {
        k: []
        for k in k_list
    }  # dictionary to store accuracies for each k
    ...
    for i, (train_index,
        ...
        all_distances = []
        for test_idx, (test_graph, ground_truth_label) in enumerate(zip(X_test, y_test)):
            # Get test graph features
            test_graph_obj = graphs_list[test_index[test_idx]]
            test_features = np.array([test_graph_obj.nodes[n]['attributes'] for n in test_graph_obj.nodes()])
            
            distances = []
            for train_idx, (train_graph, train_label) in enumerate(zip(X_train, y_train)):
                # Get train graph features
                train_graph_obj = graphs_list[train_index[train_idx]]
                train_features = np.array([train_graph_obj.nodes[n]['attributes'] for n in train_graph_obj.nodes()])
                ...
                fgw, log = ot.gromov.fused_gromov_wasserstein(M,
                                                       test_graph,
                                                       train_graph,
                                                       loss_fun='square_loss',
                                                       log=True,
