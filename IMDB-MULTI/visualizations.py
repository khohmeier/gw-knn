...
class GCNResultsVisualizer:
    ...
    def plot_cross_validation_results(self, save_path="cv_results.png"):
        ...
        # Extract metrics
        metrics = ['overall_accuracy', 'class_0_accuracy', 'class_1_accuracy', 'class_2_accuracy', 'f1_score']
    ...
    def plot_roc_curve(self, y_true, y_scores, save_path="roc_curve.png"):
        ...
        for i, color in zip(range(n_classes), colors):
            plt.plot(fpr[i], tpr[i], color=color, lw=2,
                    label=f'ROC curve Class {i+1} (AUC = {roc_auc[i]:.4f})')
    ...
    def plot_sample_graphs(self, graphs, labels, num_samples=6, save_path="sample_graphs.png"):
        ...
        for i, idx in enumerate(indices):
            ...
            # Draw graph with different colors for each class
            color_map = {1: 'lightblue', 2: 'lightcoral', 3: 'lightgreen'}
            node_color = color_map.get(label, 'lightgray')  # Convert back to original labels {1,2,3}
            ...
            axes[i].set_title(f'Graph {idx} (Label: {label})')  # Show original labels {1,2,3}
